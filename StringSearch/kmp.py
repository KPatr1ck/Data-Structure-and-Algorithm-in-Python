...
def generate_next(pattern):
    ...
    注意：
    1. kmp原理与fsm在本质上是一样的，都是切换状态
    2. fsm *考虑上当前从main中读入的字符*，从长到短截取后缀子串，去pattern
       中找是否存在前缀子串与其匹配(也就是去找能不能回到之前的状态)
    3. kmp是在当前字符不匹配时，*不考虑当前的字符*，而是在之前的匹配中，从
       长到短截取后缀子串，去pattern中找是否存在前缀子串与其匹配，然后将
       *匹配字符* 对齐，继续进行比较
    :param pattern:
    :return:
    """
